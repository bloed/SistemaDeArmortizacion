package Datos;

public class Constantes {
  public static String COLON = "Colon";
  public static String DOLAR = "Dolar";
  public static String ALEMAN = "Aleman";
  public static String FRANCES = "Frances";
  public static String AMERICANO = "Americano";
  public static String CLASEMONEDA = "Modelo.Monedas.";
  public static String CLASESISTEMA = "Modelo.Sistemas.";
  public static String URLBCCR = "http://indicadoreseconomicos.bccr.fi.cr/indicadoreseconomicos/WebServices/wsindicadoreseconomicos.asmx/ObtenerIndicadoresEconomicos?";
                                 
  public static CharSequence ENCABEZADOCSV = "NombreCliente;TipoSistema;Deuda;Plazo;Interes;TipoCambio;FechaChucky";
}
